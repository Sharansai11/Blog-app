    ...
    const writeComment = async (commentObj) => {
        try {
            ...
            if (res.data.message === "Comment posted") {
                // Update the currentArticle state with the new comment
                setCurrentArticle(prevState => ({
                    ...prevState,
                    comments: [...prevState.comments, commentObj] // Add the new comment to the existing comments array
                }));
                setComment(res.data.message);
                navigate(`../article/${res.data.payload.articleId}`, {
                    state: res.data.payload,
                });
                console.log(res.data.payload.comments);
            } else {
                    ...
                    <div>
                        ...
                        <div className="comments my-4">
                            {state.comments.length === 0 ? (
                                <p className="display-2">No comments yet...</p>
                            ) : (
